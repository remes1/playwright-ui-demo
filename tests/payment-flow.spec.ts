...
test.describe('Payment workflows', () => {
    ...
    test('Successful payment flow', async ({ e2e, checkout }) => {
        ...
        await expect(checkout.orderPlacedConfirmation).toHaveText(assertions.orderPlacedConfirmation);
    });
    ...
    test('Failed payment flow', async ({ e2e, checkout }) => {
        ...
        await expect(checkout.paymentError).toHaveText(assertions.paymentError);
    });
